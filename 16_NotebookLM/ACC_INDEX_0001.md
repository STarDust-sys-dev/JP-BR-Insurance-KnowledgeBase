# ACC_INDEX_0001 - 傷害保険対応ナビゲーション

## Título

ACC_INDEX_0001 - 傷害保険対応ナビゲーション

## Resumo

傷害保険、労災保険、通勤災害、事故通知、診断書、診療明細書、入院保険金、手術保険金、免責日数、免責事由を、ブラジル人顧客へ説明するための検索導線。日本人保険募集人が日本語で状況を整理し、必要に応じてポルトガル語の説明文とカタカナ読みを使って顧客対応できるように設計されている。

## Palavras-chave

傷害保険, 労災保険, 通勤災害, 事故通知, 保険金請求, 請求書類, 診断書, 診療明細書, 領収書, 事故証明書, 入院保険金, 通院保険金, 手術保険金, 死亡保険金, 後遺障害保険金, 免責日数, 免責事由, 急激, 偶然, 外来, 支払査定, 保険証券, 約款, ブラジル人顧客, português brasileiro, カタカナ読み

## Categorias

- ACC
- NotebookLM
- 日本人保険募集人
- ブラジル人顧客対応
- Português brasileiro com leitura em katakana

## Conteúdo

### 検索方針

- まず事故の場面を確認する: 仕事中、通勤中、日常生活、入院、手術、書類提出。
- 次に `ACC-0001` から `ACC-0010` で用語を確認する。
- 顧客への短い回答が必要な場合は `FAQ-ACC` を確認する。
- 会話例が必要な場合は `DIALOGUE-ACC` を確認する。
- 判断や説明の流れが難しい場合は `CASE-ACC` を確認する。
- 支払可否、補償、労災該当性、免責該当性、支払日数、支払金額は断定しない。

### NotebookLMでの自然な検索例

- ブラジル人のお客様に傷害保険をどう説明しますか。
- 仕事中の事故は労災保険と民間保険をどう分けますか。
- 通勤中の事故をポルトガル語で説明したいです。
- 事故後にどの書類が必要か説明したいです。
- 診断書と診療明細書の違いを説明したいです。
- 免責日数をブラジル人顧客にどう説明しますか。
- 飲酒中の事故で支払可否を断定しない説明をしたいです。

### 推奨確認順序

1. `ACC-0001` で民間の傷害保険を確認する。
2. `ACC-0002` と `ACC-0003` で労災保険と通勤災害を分ける。
3. `ACC-0004` で事故通知の流れを確認する。
4. `ACC-0005` と `ACC-0006` で必要書類を確認する。
5. `ACC-0007` と `ACC-0008` で入院・手術の給付を確認する。
6. `ACC-0009` と `ACC-0010` で免責日数と免責事由を確認する。

### Verbetes

- ACC-0001 - 傷害保険 - Seguro de acidentes pessoais
- ACC-0002 - 労災保険 - Seguro de acidente de trabalho
- ACC-0003 - 通勤災害 - Acidente no trajeto de trabalho
- ACC-0004 - 事故通知 - Aviso de acidente
- ACC-0005 - 診断書 - Atestado médico
- ACC-0006 - 診療明細書 - Detalhamento do atendimento médico
- ACC-0007 - 入院保険金 - Benefício por internação causada por acidente
- ACC-0008 - 手術保険金 - Benefício por cirurgia causada por acidente
- ACC-0009 - 免責日数 - Franquia temporal em dias
- ACC-0010 - 免責事由 - Exclusões contratuais
- ACC-0011 - 通院保険金 - Benefício por tratamento ambulatorial causado por acidente
- ACC-0012 - 死亡保険金 - Benefício por falecimento causado por acidente
- ACC-0013 - 後遺障害保険金 - Benefício por sequela permanente causada por acidente
- ACC-0014 - 保険金請求書 - Formulário de pedido de benefício do seguro
- ACC-0015 - 事故証明書 - Certificado de acidente
- ACC-0016 - 領収書 - Recibo
- ACC-0017 - 急激 - Súbito
- ACC-0018 - 偶然 - Acidental
- ACC-0019 - 外来 - Externo
- ACC-0020 - 事故証明 - Comprovação do acidente

### FAQs

- FAQ-ACC-0001
- FAQ-ACC-0002
- FAQ-ACC-0003
- FAQ-ACC-0004
- FAQ-ACC-0005
- FAQ-ACC-0006
- FAQ-ACC-0007
- FAQ-ACC-0008
- FAQ-ACC-0009
- FAQ-ACC-0010
- FAQ-ACC-0011
- FAQ-ACC-0012
- FAQ-ACC-0013
- FAQ-ACC-0014
- FAQ-ACC-0015
- FAQ-ACC-0016
- FAQ-ACC-0017
- FAQ-ACC-0018
- FAQ-ACC-0019
- FAQ-ACC-0020

### Diálogos

- DIALOGUE-ACC-0001
- DIALOGUE-ACC-0002
- DIALOGUE-ACC-0003
- DIALOGUE-ACC-0004
- DIALOGUE-ACC-0005
- DIALOGUE-ACC-0006
- DIALOGUE-ACC-0007
- DIALOGUE-ACC-0008
- DIALOGUE-ACC-0009
- DIALOGUE-ACC-0010

### Estudos de caso

- CASE-ACC-0001
- CASE-ACC-0002
- CASE-ACC-0003
- CASE-ACC-0004
- CASE-ACC-0005
- CASE-ACC-0006
- CASE-ACC-0007
- CASE-ACC-0008
- CASE-ACC-0009
- CASE-ACC-0010

### Frases prontas essenciais

#### ポルトガル語で伝える内容

Primeiro vamos separar o seguro de acidente de trabalho e o seguro privado de acidentes.

#### カタカナ読み

プリメイロ ヴァモス セパラール オ セグーロ ジ アシデンチ ジ トラバーリョ イ オ セグーロ プリヴァード ジ アシデンチス

日本語確認用: 労災保険と民間の傷害保険を分けて確認する。

#### ポルトガル語で伝える内容

Eu não posso prometer pagamento nem prazo antes da análise da seguradora.

#### カタカナ読み

エウ ナォン ポッソ プロメテール パガメント ネン プラゾ アンチス ダ アナリゼ ダ セグラドーラ

日本語確認用: 保険会社の査定前に支払いや時期を約束しない。

### Alertas de atendimento

- 労災保険、公的制度、民間の傷害保険を混同しない。
- 事故通知だけで支払いを約束しない。
- 診断書や診療明細書の提出だけで支払いを約束しない。
- 免責日数、免責事由、飲酒、重大な過失、法令違反に関する判断を断定しない。
- 契約条件、事故状況、提出書類、保険会社の査定を必ず分けて説明する。

## Referências

- `02_MasterDictionary/ACC/ACC-0001.md`
- `14_FAQ/ACC/FAQ-ACC-0001.md`
- `13_Dialogues/ACC/DIALOGUE-ACC-0001.md`
- `13_Dialogues/ACC/DIALOGUE-ACC-0002.md`
- `13_Dialogues/ACC/DIALOGUE-ACC-0003.md`
- `13_Dialogues/ACC/DIALOGUE-ACC-0004.md`
- `13_Dialogues/ACC/DIALOGUE-ACC-0005.md`
- `13_Dialogues/ACC/DIALOGUE-ACC-0006.md`
- `15_CaseStudies/ACC/CASE-ACC-0001.md`
- `15_CaseStudies/ACC/CASE-ACC-0002.md`
- `15_CaseStudies/ACC/CASE-ACC-0003.md`
- `15_CaseStudies/ACC/CASE-ACC-0004.md`
- `15_CaseStudies/ACC/CASE-ACC-0005.md`
- `15_CaseStudies/ACC/CASE-ACC-0006.md`
- `19_Markdown/ACC/ACC_DIALOGUES_0003_0006.md`
- `19_Markdown/ACC/ACC_CASES_0003_0006.md`
- `19_Markdown/ACC/ACC_TERMS_0021_0030.md`
- `19_Markdown/ACC/ACC_FAQ_0021_0030.md`
- `19_Markdown/ACC/ACC_DIALOGUES_0007_0010.md`
- `19_Markdown/ACC/ACC_CASES_0007_0010.md`

## Veja também

- `16_NotebookLM/ACC_NOTEBOOKLM_PACK_0001.md`
- `10_Accident/ACC_NAVIGATION_GUIDE.md`
- `19_Markdown/ACC/ACC_TERMS_0001_0010.md`
- `19_Markdown/ACC/ACC_TERMS_0011_0020.md`
- `19_Markdown/ACC/ACC_FAQ_0001_0005.md`
- `19_Markdown/ACC/ACC_FAQ_0006_0010.md`
- `19_Markdown/ACC/ACC_FAQ_0011_0020.md`
- `19_Markdown/ACC/ACC_DIALOGUES_0001_0002.md`
- `19_Markdown/ACC/ACC_CASES_0001_0002.md`
