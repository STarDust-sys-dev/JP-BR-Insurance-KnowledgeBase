# AUTO_CASES_0021_0028 - Quinto e sexto lotes de estudos de caso AUTO


---

# CASE-AUTO-0021 - Cliente não sabe descrever as circunstâncias do acidente

## Controle

- Versão: 0.1
- Data: 2026-06-25
- Autor: JP-BR Insurance Knowledge Base
- Histórico: Criação inicial.

## Código

CASE-AUTO-0021

## Produto ou categoria

AUTO

## Situação

Cliente não sabe descrever as circunstâncias do acidente.

## Problema

Cliente pode tomar decisão incorreta por falta de compreensão técnica, documentação incompleta ou comunicação informal com terceiros.

## Análise

O atendimento deve separar fatos, documentos, responsabilidade, cobertura e decisão da seguradora. O corretor deve orientar sem prometer resultado antes da análise formal.

## Orientação ao corretor

Coletar dados essenciais, registrar a situação, conectar o cliente aos documentos corretos e confirmar próximos passos com a seguradora.

## Como explicar ao cliente brasileiro

Vamos organizar as informações e documentos antes de decidir qualquer coisa. Assim evitamos erro e ajudamos a seguradora a analisar corretamente.

## Resultado esperado

Cliente entende o procedimento, evita acordo ou decisão precipitada e segue o fluxo correto de atendimento.

## Termos relacionados

- AUTO-0041 - 事故状況 - Circunstâncias do acidente
- AUTO-0042 - 相手方 - Outra parte envolvida

## FAQs relacionadas

- FAQ-AUTO-0038
- FAQ-AUTO-0039

## Diálogos relacionados

- DIALOGUE-AUTO-0015

## Referências cruzadas

- AUTO-0041
- AUTO-0042
- FAQ-AUTO-0038
- FAQ-AUTO-0039
- DIALOGUE-AUTO-0015

## Referências

- Documento interno: `EDITORIAL_RULES.md`

## Tags

- case
- auto
- sinistro
- atendimento

## Veja também

- `14_FAQ/AUTO/FAQ-AUTO-0038.md`

## Histórico de revisão

| Data | Versão | Autor | Alteração |
| --- | --- | --- | --- |
| 2026-06-25 | 0.1 | JP-BR Insurance Knowledge Base | Criação inicial. |

---

# CASE-AUTO-0022 - Cliente discorda do percentual de culpa

## Controle

- Versão: 0.1
- Data: 2026-06-25
- Autor: JP-BR Insurance Knowledge Base
- Histórico: Criação inicial.

## Código

CASE-AUTO-0022

## Produto ou categoria

AUTO

## Situação

Cliente discorda do percentual de culpa.

## Problema

Cliente pode tomar decisão incorreta por falta de compreensão técnica, documentação incompleta ou comunicação informal com terceiros.

## Análise

O atendimento deve separar fatos, documentos, responsabilidade, cobertura e decisão da seguradora. O corretor deve orientar sem prometer resultado antes da análise formal.

## Orientação ao corretor

Coletar dados essenciais, registrar a situação, conectar o cliente aos documentos corretos e confirmar próximos passos com a seguradora.

## Como explicar ao cliente brasileiro

Vamos organizar as informações e documentos antes de decidir qualquer coisa. Assim evitamos erro e ajudamos a seguradora a analisar corretamente.

## Resultado esperado

Cliente entende o procedimento, evita acordo ou decisão precipitada e segue o fluxo correto de atendimento.

## Termos relacionados

- AUTO-0043 - 過失割合 - Percentual de culpa
- AUTO-0044 - 示談 - Acordo extrajudicial

## FAQs relacionadas

- FAQ-AUTO-0040
- FAQ-AUTO-0041

## Diálogos relacionados

- DIALOGUE-AUTO-0015
- DIALOGUE-AUTO-0016

## Referências cruzadas

- AUTO-0043
- AUTO-0044
- FAQ-AUTO-0040
- FAQ-AUTO-0041
- DIALOGUE-AUTO-0015
- DIALOGUE-AUTO-0016

## Referências

- Documento interno: `EDITORIAL_RULES.md`

## Tags

- case
- auto
- sinistro
- atendimento

## Veja também

- `14_FAQ/AUTO/FAQ-AUTO-0040.md`

## Histórico de revisão

| Data | Versão | Autor | Alteração |
| --- | --- | --- | --- |
| 2026-06-25 | 0.1 | JP-BR Insurance Knowledge Base | Criação inicial. |

---

# CASE-AUTO-0023 - Danos a terceiro exigem análise de indenização

## Controle

- Versão: 0.1
- Data: 2026-06-25
- Autor: JP-BR Insurance Knowledge Base
- Histórico: Criação inicial.

## Código

CASE-AUTO-0023

## Produto ou categoria

AUTO

## Situação

Danos a terceiro exigem análise de indenização.

## Problema

Cliente pode tomar decisão incorreta por falta de compreensão técnica, documentação incompleta ou comunicação informal com terceiros.

## Análise

O atendimento deve separar fatos, documentos, responsabilidade, cobertura e decisão da seguradora. O corretor deve orientar sem prometer resultado antes da análise formal.

## Orientação ao corretor

Coletar dados essenciais, registrar a situação, conectar o cliente aos documentos corretos e confirmar próximos passos com a seguradora.

## Como explicar ao cliente brasileiro

Vamos organizar as informações e documentos antes de decidir qualquer coisa. Assim evitamos erro e ajudamos a seguradora a analisar corretamente.

## Resultado esperado

Cliente entende o procedimento, evita acordo ou decisão precipitada e segue o fluxo correto de atendimento.

## Termos relacionados

- AUTO-0045 - 損害賠償 - Indenização por danos
- AUTO-0003 - AUTO-0003 - Termo relacionado

## FAQs relacionadas

- FAQ-AUTO-0042
- FAQ-AUTO-0003

## Diálogos relacionados

- DIALOGUE-AUTO-0016

## Referências cruzadas

- AUTO-0045
- AUTO-0003
- FAQ-AUTO-0042
- FAQ-AUTO-0003
- DIALOGUE-AUTO-0016

## Referências

- Documento interno: `EDITORIAL_RULES.md`

## Tags

- case
- auto
- sinistro
- atendimento

## Veja também

- `14_FAQ/AUTO/FAQ-AUTO-0042.md`

## Histórico de revisão

| Data | Versão | Autor | Alteração |
| --- | --- | --- | --- |
| 2026-06-25 | 0.1 | JP-BR Insurance Knowledge Base | Criação inicial. |

---

# CASE-AUTO-0024 - Seguradora solicita orçamento e avaliação de danos

## Controle

- Versão: 0.1
- Data: 2026-06-25
- Autor: JP-BR Insurance Knowledge Base
- Histórico: Criação inicial.

## Código

CASE-AUTO-0024

## Produto ou categoria

AUTO

## Situação

Seguradora solicita orçamento e avaliação de danos.

## Problema

Cliente pode tomar decisão incorreta por falta de compreensão técnica, documentação incompleta ou comunicação informal com terceiros.

## Análise

O atendimento deve separar fatos, documentos, responsabilidade, cobertura e decisão da seguradora. O corretor deve orientar sem prometer resultado antes da análise formal.

## Orientação ao corretor

Coletar dados essenciais, registrar a situação, conectar o cliente aos documentos corretos e confirmar próximos passos com a seguradora.

## Como explicar ao cliente brasileiro

Vamos organizar as informações e documentos antes de decidir qualquer coisa. Assim evitamos erro e ajudamos a seguradora a analisar corretamente.

## Resultado esperado

Cliente entende o procedimento, evita acordo ou decisão precipitada e segue o fluxo correto de atendimento.

## Termos relacionados

- AUTO-0046 - 修理見積書 - Orçamento de reparo
- AUTO-0047 - 損害調査 - Avaliação de danos

## FAQs relacionadas

- FAQ-AUTO-0043
- FAQ-AUTO-0044

## Diálogos relacionados

- DIALOGUE-AUTO-0017

## Referências cruzadas

- AUTO-0046
- AUTO-0047
- FAQ-AUTO-0043
- FAQ-AUTO-0044
- DIALOGUE-AUTO-0017

## Referências

- Documento interno: `EDITORIAL_RULES.md`

## Tags

- case
- auto
- sinistro
- atendimento

## Veja também

- `14_FAQ/AUTO/FAQ-AUTO-0043.md`

## Histórico de revisão

| Data | Versão | Autor | Alteração |
| --- | --- | --- | --- |
| 2026-06-25 | 0.1 | JP-BR Insurance Knowledge Base | Criação inicial. |

---

# CASE-AUTO-0025 - Cliente não entende diferença entre perda total e parcial

## Controle

- Versão: 0.1
- Data: 2026-06-25
- Autor: JP-BR Insurance Knowledge Base
- Histórico: Criação inicial.

## Código

CASE-AUTO-0025

## Produto ou categoria

AUTO

## Situação

Cliente não entende diferença entre perda total e parcial.

## Problema

Cliente pode tomar decisão incorreta por falta de compreensão técnica, documentação incompleta ou comunicação informal com terceiros.

## Análise

O atendimento deve separar fatos, documentos, responsabilidade, cobertura e decisão da seguradora. O corretor deve orientar sem prometer resultado antes da análise formal.

## Orientação ao corretor

Coletar dados essenciais, registrar a situação, conectar o cliente aos documentos corretos e confirmar próximos passos com a seguradora.

## Como explicar ao cliente brasileiro

Vamos organizar as informações e documentos antes de decidir qualquer coisa. Assim evitamos erro e ajudamos a seguradora a analisar corretamente.

## Resultado esperado

Cliente entende o procedimento, evita acordo ou decisão precipitada e segue o fluxo correto de atendimento.

## Termos relacionados

- AUTO-0048 - 全損 - Perda total
- AUTO-0049 - 分損 - Perda parcial

## FAQs relacionadas

- FAQ-AUTO-0045
- FAQ-AUTO-0046

## Diálogos relacionados

- DIALOGUE-AUTO-0017

## Referências cruzadas

- AUTO-0048
- AUTO-0049
- FAQ-AUTO-0045
- FAQ-AUTO-0046
- DIALOGUE-AUTO-0017

## Referências

- Documento interno: `EDITORIAL_RULES.md`

## Tags

- case
- auto
- sinistro
- atendimento

## Veja também

- `14_FAQ/AUTO/FAQ-AUTO-0045.md`

## Histórico de revisão

| Data | Versão | Autor | Alteração |
| --- | --- | --- | --- |
| 2026-06-25 | 0.1 | JP-BR Insurance Knowledge Base | Criação inicial. |

---

# CASE-AUTO-0026 - Veículo precisa de guincho e oficina após pane ou acidente

## Controle

- Versão: 0.1
- Data: 2026-06-25
- Autor: JP-BR Insurance Knowledge Base
- Histórico: Criação inicial.

## Código

CASE-AUTO-0026

## Produto ou categoria

AUTO

## Situação

Veículo precisa de guincho e oficina após pane ou acidente.

## Problema

Cliente pode tomar decisão incorreta por falta de compreensão técnica, documentação incompleta ou comunicação informal com terceiros.

## Análise

O atendimento deve separar fatos, documentos, responsabilidade, cobertura e decisão da seguradora. O corretor deve orientar sem prometer resultado antes da análise formal.

## Orientação ao corretor

Coletar dados essenciais, registrar a situação, conectar o cliente aos documentos corretos e confirmar próximos passos com a seguradora.

## Como explicar ao cliente brasileiro

Vamos organizar as informações e documentos antes de decidir qualquer coisa. Assim evitamos erro e ajudamos a seguradora a analisar corretamente.

## Resultado esperado

Cliente entende o procedimento, evita acordo ou decisão precipitada e segue o fluxo correto de atendimento.

## Termos relacionados

- AUTO-0050 - レッカー搬送 - Remoção por guincho
- AUTO-0051 - 修理工場 - Oficina de reparo
- AUTO-0052 - 保険会社 - Seguradora
- AUTO-0053 - 保険代理店 - Agência ou corretora de seguros

## FAQs relacionadas

- FAQ-AUTO-0047
- FAQ-AUTO-0048
- FAQ-AUTO-0049
- FAQ-AUTO-0050

## Diálogos relacionados

- DIALOGUE-AUTO-0018

## Referências cruzadas

- AUTO-0050
- AUTO-0051
- AUTO-0052
- AUTO-0053
- FAQ-AUTO-0047
- FAQ-AUTO-0048
- FAQ-AUTO-0049
- FAQ-AUTO-0050
- DIALOGUE-AUTO-0018

## Referências

- Documento interno: `EDITORIAL_RULES.md`

## Tags

- case
- auto
- sinistro
- atendimento

## Veja também

- `14_FAQ/AUTO/FAQ-AUTO-0047.md`

## Histórico de revisão

| Data | Versão | Autor | Alteração |
| --- | --- | --- | --- |
| 2026-06-25 | 0.1 | JP-BR Insurance Knowledge Base | Criação inicial. |

---

# CASE-AUTO-0027 - Informação incorreta na contratação causa risco contratual

## Controle

- Versão: 0.1
- Data: 2026-06-25
- Autor: JP-BR Insurance Knowledge Base
- Histórico: Criação inicial.

## Código

CASE-AUTO-0027

## Produto ou categoria

AUTO

## Situação

Informação incorreta na contratação causa risco contratual.

## Problema

Cliente pode tomar decisão incorreta por falta de compreensão técnica, documentação incompleta ou comunicação informal com terceiros.

## Análise

O atendimento deve separar fatos, documentos, responsabilidade, cobertura e decisão da seguradora. O corretor deve orientar sem prometer resultado antes da análise formal.

## Orientação ao corretor

Coletar dados essenciais, registrar a situação, conectar o cliente aos documentos corretos e confirmar próximos passos com a seguradora.

## Como explicar ao cliente brasileiro

Vamos organizar as informações e documentos antes de decidir qualquer coisa. Assim evitamos erro e ajudamos a seguradora a analisar corretamente.

## Resultado esperado

Cliente entende o procedimento, evita acordo ou decisão precipitada e segue o fluxo correto de atendimento.

## Termos relacionados

- AUTO-0054 - 契約申込書 - Proposta de contratação
- AUTO-0055 - 告知事項 - Informações declaradas na contratação
- AUTO-0056 - 通知義務 - Obrigação de comunicar alterações
- AUTO-0057 - 変更手続き - Procedimento de alteração

## FAQs relacionadas

- FAQ-AUTO-0051
- FAQ-AUTO-0052
- FAQ-AUTO-0053
- FAQ-AUTO-0054

## Diálogos relacionados

- DIALOGUE-AUTO-0019

## Referências cruzadas

- AUTO-0054
- AUTO-0055
- AUTO-0056
- AUTO-0057
- FAQ-AUTO-0051
- FAQ-AUTO-0052
- FAQ-AUTO-0053
- FAQ-AUTO-0054
- DIALOGUE-AUTO-0019

## Referências

- Documento interno: `EDITORIAL_RULES.md`

## Tags

- case
- auto
- sinistro
- atendimento

## Veja também

- `14_FAQ/AUTO/FAQ-AUTO-0051.md`

## Histórico de revisão

| Data | Versão | Autor | Alteração |
| --- | --- | --- | --- |
| 2026-06-25 | 0.1 | JP-BR Insurance Knowledge Base | Criação inicial. |

---

# CASE-AUTO-0028 - Cliente precisa comprovar apólice, renovação ou interrupção

## Controle

- Versão: 0.1
- Data: 2026-06-25
- Autor: JP-BR Insurance Knowledge Base
- Histórico: Criação inicial.

## Código

CASE-AUTO-0028

## Produto ou categoria

AUTO

## Situação

Cliente precisa comprovar apólice, renovação ou interrupção.

## Problema

Cliente pode tomar decisão incorreta por falta de compreensão técnica, documentação incompleta ou comunicação informal com terceiros.

## Análise

O atendimento deve separar fatos, documentos, responsabilidade, cobertura e decisão da seguradora. O corretor deve orientar sem prometer resultado antes da análise formal.

## Orientação ao corretor

Coletar dados essenciais, registrar a situação, conectar o cliente aos documentos corretos e confirmar próximos passos com a seguradora.

## Como explicar ao cliente brasileiro

Vamos organizar as informações e documentos antes de decidir qualquer coisa. Assim evitamos erro e ajudamos a seguradora a analisar corretamente.

## Resultado esperado

Cliente entende o procedimento, evita acordo ou decisão precipitada e segue o fluxo correto de atendimento.

## Termos relacionados

- AUTO-0058 - 保険証券 - Apólice de seguro
- AUTO-0059 - 継続証 - Comprovante de renovação
- AUTO-0060 - 中断証明書 - Certificado de interrupção

## FAQs relacionadas

- FAQ-AUTO-0055
- FAQ-AUTO-0056
- FAQ-AUTO-0057

## Diálogos relacionados

- DIALOGUE-AUTO-0020

## Referências cruzadas

- AUTO-0058
- AUTO-0059
- AUTO-0060
- FAQ-AUTO-0055
- FAQ-AUTO-0056
- FAQ-AUTO-0057
- DIALOGUE-AUTO-0020

## Referências

- Documento interno: `EDITORIAL_RULES.md`

## Tags

- case
- auto
- sinistro
- atendimento

## Veja também

- `14_FAQ/AUTO/FAQ-AUTO-0055.md`

## Histórico de revisão

| Data | Versão | Autor | Alteração |
| --- | --- | --- | --- |
| 2026-06-25 | 0.1 | JP-BR Insurance Knowledge Base | Criação inicial. |
