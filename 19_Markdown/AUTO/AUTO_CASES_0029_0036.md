# AUTO_CASES_0029_0036 - Sétimo e oitavo lotes de estudos de caso AUTO


---

# CASE-AUTO-0029 - Cliente acredita que qualquer acidente está coberto

## Controle

- Versão: 0.1
- Data: 2026-06-25
- Autor: JP-BR Insurance Knowledge Base
- Histórico: Criação inicial.

## Código

CASE-AUTO-0029

## Produto ou categoria

AUTO

## Situação

Cliente acredita que qualquer acidente está coberto.

## Problema

Cliente pode presumir cobertura automática em situação que depende de exclusões, restrições ou contratação específica.

## Análise

Situações especiais devem ser avaliadas com base na apólice, nas exclusões, nos fatos e na decisão da seguradora. O corretor deve evitar confirmação informal de cobertura.

## Orientação ao corretor

Coletar fatos, documentos e dados do condutor ou evento. Explicar que a cobertura depende do contrato e da análise formal da seguradora.

## Como explicar ao cliente brasileiro

Nem toda situação está coberta automaticamente. Vamos verificar seu contrato e explicar com clareza antes de qualquer decisão.

## Resultado esperado

Cliente entende o limite da cobertura e segue o procedimento correto sem criar expectativa indevida.

## Termos relacionados

- AUTO-0061 - 免責事項 - Exclusões e limitações de cobertura
- AUTO-0062 - 補償対象外 - Fora da cobertura

## FAQs relacionadas

- FAQ-AUTO-0058
- FAQ-AUTO-0059

## Diálogos relacionados

- DIALOGUE-AUTO-0021

## Referências cruzadas

- AUTO-0061
- AUTO-0062
- FAQ-AUTO-0058
- FAQ-AUTO-0059
- DIALOGUE-AUTO-0021

## Referências

- Documento interno: `EDITORIAL_RULES.md`

## Tags

- case
- auto
- exclusao
- cobertura

## Veja também

- `14_FAQ/AUTO/FAQ-AUTO-0058.md`

## Histórico de revisão

| Data | Versão | Autor | Alteração |
| --- | --- | --- | --- |
| 2026-06-25 | 0.1 | JP-BR Insurance Knowledge Base | Criação inicial. |

---

# CASE-AUTO-0030 - Acidente com suspeita de álcool ou falta de habilitação

## Controle

- Versão: 0.1
- Data: 2026-06-25
- Autor: JP-BR Insurance Knowledge Base
- Histórico: Criação inicial.

## Código

CASE-AUTO-0030

## Produto ou categoria

AUTO

## Situação

Acidente com suspeita de álcool ou falta de habilitação.

## Problema

Cliente pode presumir cobertura automática em situação que depende de exclusões, restrições ou contratação específica.

## Análise

Situações especiais devem ser avaliadas com base na apólice, nas exclusões, nos fatos e na decisão da seguradora. O corretor deve evitar confirmação informal de cobertura.

## Orientação ao corretor

Coletar fatos, documentos e dados do condutor ou evento. Explicar que a cobertura depende do contrato e da análise formal da seguradora.

## Como explicar ao cliente brasileiro

Nem toda situação está coberta automaticamente. Vamos verificar seu contrato e explicar com clareza antes de qualquer decisão.

## Resultado esperado

Cliente entende o limite da cobertura e segue o procedimento correto sem criar expectativa indevida.

## Termos relacionados

- AUTO-0063 - 飲酒運転 - Direção sob efeito de álcool
- AUTO-0064 - 無免許運転 - Direção sem habilitação

## FAQs relacionadas

- FAQ-AUTO-0060
- FAQ-AUTO-0061

## Diálogos relacionados

- DIALOGUE-AUTO-0021

## Referências cruzadas

- AUTO-0063
- AUTO-0064
- FAQ-AUTO-0060
- FAQ-AUTO-0061
- DIALOGUE-AUTO-0021

## Referências

- Documento interno: `EDITORIAL_RULES.md`

## Tags

- case
- auto
- exclusao
- cobertura

## Veja também

- `14_FAQ/AUTO/FAQ-AUTO-0060.md`

## Histórico de revisão

| Data | Versão | Autor | Alteração |
| --- | --- | --- | --- |
| 2026-06-25 | 0.1 | JP-BR Insurance Knowledge Base | Criação inicial. |

---

# CASE-AUTO-0031 - Dano com possível culpa grave ou ato intencional

## Controle

- Versão: 0.1
- Data: 2026-06-25
- Autor: JP-BR Insurance Knowledge Base
- Histórico: Criação inicial.

## Código

CASE-AUTO-0031

## Produto ou categoria

AUTO

## Situação

Dano com possível culpa grave ou ato intencional.

## Problema

Cliente pode presumir cobertura automática em situação que depende de exclusões, restrições ou contratação específica.

## Análise

Situações especiais devem ser avaliadas com base na apólice, nas exclusões, nos fatos e na decisão da seguradora. O corretor deve evitar confirmação informal de cobertura.

## Orientação ao corretor

Coletar fatos, documentos e dados do condutor ou evento. Explicar que a cobertura depende do contrato e da análise formal da seguradora.

## Como explicar ao cliente brasileiro

Nem toda situação está coberta automaticamente. Vamos verificar seu contrato e explicar com clareza antes de qualquer decisão.

## Resultado esperado

Cliente entende o limite da cobertura e segue o procedimento correto sem criar expectativa indevida.

## Termos relacionados

- AUTO-0065 - 重大な過失 - Culpa grave
- AUTO-0066 - 故意 - Ato intencional

## FAQs relacionadas

- FAQ-AUTO-0062
- FAQ-AUTO-0063

## Diálogos relacionados

- DIALOGUE-AUTO-0022

## Referências cruzadas

- AUTO-0065
- AUTO-0066
- FAQ-AUTO-0062
- FAQ-AUTO-0063
- DIALOGUE-AUTO-0022

## Referências

- Documento interno: `EDITORIAL_RULES.md`

## Tags

- case
- auto
- exclusao
- cobertura

## Veja também

- `14_FAQ/AUTO/FAQ-AUTO-0062.md`

## Histórico de revisão

| Data | Versão | Autor | Alteração |
| --- | --- | --- | --- |
| 2026-06-25 | 0.1 | JP-BR Insurance Knowledge Base | Criação inicial. |

---

# CASE-AUTO-0032 - Veículo danificado por tufão ou inundação

## Controle

- Versão: 0.1
- Data: 2026-06-25
- Autor: JP-BR Insurance Knowledge Base
- Histórico: Criação inicial.

## Código

CASE-AUTO-0032

## Produto ou categoria

AUTO

## Situação

Veículo danificado por tufão ou inundação.

## Problema

Cliente pode presumir cobertura automática em situação que depende de exclusões, restrições ou contratação específica.

## Análise

Situações especiais devem ser avaliadas com base na apólice, nas exclusões, nos fatos e na decisão da seguradora. O corretor deve evitar confirmação informal de cobertura.

## Orientação ao corretor

Coletar fatos, documentos e dados do condutor ou evento. Explicar que a cobertura depende do contrato e da análise formal da seguradora.

## Como explicar ao cliente brasileiro

Nem toda situação está coberta automaticamente. Vamos verificar seu contrato e explicar com clareza antes de qualquer decisão.

## Resultado esperado

Cliente entende o limite da cobertura e segue o procedimento correto sem criar expectativa indevida.

## Termos relacionados

- AUTO-0068 - 台風 - Tufão
- AUTO-0069 - 洪水 - Inundação
- AUTO-0073 - 自然災害 - Desastre natural

## FAQs relacionadas

- FAQ-AUTO-0065
- FAQ-AUTO-0066
- FAQ-AUTO-0070

## Diálogos relacionados

- DIALOGUE-AUTO-0023

## Referências cruzadas

- AUTO-0068
- AUTO-0069
- AUTO-0073
- FAQ-AUTO-0065
- FAQ-AUTO-0066
- FAQ-AUTO-0070
- DIALOGUE-AUTO-0023

## Referências

- Documento interno: `EDITORIAL_RULES.md`

## Tags

- case
- auto
- exclusao
- cobertura

## Veja também

- `14_FAQ/AUTO/FAQ-AUTO-0065.md`

## Histórico de revisão

| Data | Versão | Autor | Alteração |
| --- | --- | --- | --- |
| 2026-06-25 | 0.1 | JP-BR Insurance Knowledge Base | Criação inicial. |

---

# CASE-AUTO-0033 - Cliente pergunta sobre dano por terremoto

## Controle

- Versão: 0.1
- Data: 2026-06-25
- Autor: JP-BR Insurance Knowledge Base
- Histórico: Criação inicial.

## Código

CASE-AUTO-0033

## Produto ou categoria

AUTO

## Situação

Cliente pergunta sobre dano por terremoto.

## Problema

Cliente pode presumir cobertura automática em situação que depende de exclusões, restrições ou contratação específica.

## Análise

Situações especiais devem ser avaliadas com base na apólice, nas exclusões, nos fatos e na decisão da seguradora. O corretor deve evitar confirmação informal de cobertura.

## Orientação ao corretor

Coletar fatos, documentos e dados do condutor ou evento. Explicar que a cobertura depende do contrato e da análise formal da seguradora.

## Como explicar ao cliente brasileiro

Nem toda situação está coberta automaticamente. Vamos verificar seu contrato e explicar com clareza antes de qualquer decisão.

## Resultado esperado

Cliente entende o limite da cobertura e segue o procedimento correto sem criar expectativa indevida.

## Termos relacionados

- AUTO-0067 - 地震 - Terremoto
- AUTO-0073 - 自然災害 - Desastre natural

## FAQs relacionadas

- FAQ-AUTO-0064
- FAQ-AUTO-0070

## Diálogos relacionados

- DIALOGUE-AUTO-0023

## Referências cruzadas

- AUTO-0067
- AUTO-0073
- FAQ-AUTO-0064
- FAQ-AUTO-0070
- DIALOGUE-AUTO-0023

## Referências

- Documento interno: `EDITORIAL_RULES.md`

## Tags

- case
- auto
- exclusao
- cobertura

## Veja também

- `14_FAQ/AUTO/FAQ-AUTO-0064.md`

## Histórico de revisão

| Data | Versão | Autor | Alteração |
| --- | --- | --- | --- |
| 2026-06-25 | 0.1 | JP-BR Insurance Knowledge Base | Criação inicial. |

---

# CASE-AUTO-0034 - Roubo, vandalismo ou dano por pedra no veículo

## Controle

- Versão: 0.1
- Data: 2026-06-25
- Autor: JP-BR Insurance Knowledge Base
- Histórico: Criação inicial.

## Código

CASE-AUTO-0034

## Produto ou categoria

AUTO

## Situação

Roubo, vandalismo ou dano por pedra no veículo.

## Problema

Cliente pode presumir cobertura automática em situação que depende de exclusões, restrições ou contratação específica.

## Análise

Situações especiais devem ser avaliadas com base na apólice, nas exclusões, nos fatos e na decisão da seguradora. O corretor deve evitar confirmação informal de cobertura.

## Orientação ao corretor

Coletar fatos, documentos e dados do condutor ou evento. Explicar que a cobertura depende do contrato e da análise formal da seguradora.

## Como explicar ao cliente brasileiro

Nem toda situação está coberta automaticamente. Vamos verificar seu contrato e explicar com clareza antes de qualquer decisão.

## Resultado esperado

Cliente entende o limite da cobertura e segue o procedimento correto sem criar expectativa indevida.

## Termos relacionados

- AUTO-0070 - 盗難 - Roubo ou furto do veículo
- AUTO-0071 - いたずら - Vandalismo
- AUTO-0072 - 飛び石 - Dano por pedra lançada

## FAQs relacionadas

- FAQ-AUTO-0067
- FAQ-AUTO-0068
- FAQ-AUTO-0069

## Diálogos relacionados

- DIALOGUE-AUTO-0024

## Referências cruzadas

- AUTO-0070
- AUTO-0071
- AUTO-0072
- FAQ-AUTO-0067
- FAQ-AUTO-0068
- FAQ-AUTO-0069
- DIALOGUE-AUTO-0024

## Referências

- Documento interno: `EDITORIAL_RULES.md`

## Tags

- case
- auto
- exclusao
- cobertura

## Veja também

- `14_FAQ/AUTO/FAQ-AUTO-0067.md`

## Histórico de revisão

| Data | Versão | Autor | Alteração |
| --- | --- | --- | --- |
| 2026-06-25 | 0.1 | JP-BR Insurance Knowledge Base | Criação inicial. |

---

# CASE-AUTO-0035 - Motorista fora da restrição familiar ou de casal

## Controle

- Versão: 0.1
- Data: 2026-06-25
- Autor: JP-BR Insurance Knowledge Base
- Histórico: Criação inicial.

## Código

CASE-AUTO-0035

## Produto ou categoria

AUTO

## Situação

Motorista fora da restrição familiar ou de casal.

## Problema

Cliente pode presumir cobertura automática em situação que depende de exclusões, restrições ou contratação específica.

## Análise

Situações especiais devem ser avaliadas com base na apólice, nas exclusões, nos fatos e na decisão da seguradora. O corretor deve evitar confirmação informal de cobertura.

## Orientação ao corretor

Coletar fatos, documentos e dados do condutor ou evento. Explicar que a cobertura depende do contrato e da análise formal da seguradora.

## Como explicar ao cliente brasileiro

Nem toda situação está coberta automaticamente. Vamos verificar seu contrato e explicar com clareza antes de qualquer decisão.

## Resultado esperado

Cliente entende o limite da cobertura e segue o procedimento correto sem criar expectativa indevida.

## Termos relacionados

- AUTO-0074 - 使用者 - Usuário do veículo
- AUTO-0075 - 記名被保険者 - Segurado principal nomeado
- AUTO-0076 - 家族限定 - Restrição a familiares
- AUTO-0077 - 夫婦限定 - Restrição a casal

## FAQs relacionadas

- FAQ-AUTO-0071
- FAQ-AUTO-0072
- FAQ-AUTO-0073
- FAQ-AUTO-0074

## Diálogos relacionados

- DIALOGUE-AUTO-0025

## Referências cruzadas

- AUTO-0074
- AUTO-0075
- AUTO-0076
- AUTO-0077
- FAQ-AUTO-0071
- FAQ-AUTO-0072
- FAQ-AUTO-0073
- FAQ-AUTO-0074
- DIALOGUE-AUTO-0025

## Referências

- Documento interno: `EDITORIAL_RULES.md`

## Tags

- case
- auto
- exclusao
- cobertura

## Veja também

- `14_FAQ/AUTO/FAQ-AUTO-0071.md`

## Histórico de revisão

| Data | Versão | Autor | Alteração |
| --- | --- | --- | --- |
| 2026-06-25 | 0.1 | JP-BR Insurance Knowledge Base | Criação inicial. |

---

# CASE-AUTO-0036 - Cliente presume que coberturas adicionais já estão incluídas

## Controle

- Versão: 0.1
- Data: 2026-06-25
- Autor: JP-BR Insurance Knowledge Base
- Histórico: Criação inicial.

## Código

CASE-AUTO-0036

## Produto ou categoria

AUTO

## Situação

Cliente presume que coberturas adicionais já estão incluídas.

## Problema

Cliente pode presumir cobertura automática em situação que depende de exclusões, restrições ou contratação específica.

## Análise

Situações especiais devem ser avaliadas com base na apólice, nas exclusões, nos fatos e na decisão da seguradora. O corretor deve evitar confirmação informal de cobertura.

## Orientação ao corretor

Coletar fatos, documentos e dados do condutor ou evento. Explicar que a cobertura depende do contrato e da análise formal da seguradora.

## Como explicar ao cliente brasileiro

Nem toda situação está coberta automaticamente. Vamos verificar seu contrato e explicar com clareza antes de qualquer decisão.

## Resultado esperado

Cliente entende o limite da cobertura e segue o procedimento correto sem criar expectativa indevida.

## Termos relacionados

- AUTO-0078 - 他車運転特約 - Cobertura adicional para dirigir outro veículo
- AUTO-0079 - ファミリーバイク特約 - Cobertura adicional para moto pequena da família
- AUTO-0080 - 個人賠償責任特約 - Cobertura adicional de responsabilidade civil pessoal

## FAQs relacionadas

- FAQ-AUTO-0075
- FAQ-AUTO-0076
- FAQ-AUTO-0077

## Diálogos relacionados

- DIALOGUE-AUTO-0026

## Referências cruzadas

- AUTO-0078
- AUTO-0079
- AUTO-0080
- FAQ-AUTO-0075
- FAQ-AUTO-0076
- FAQ-AUTO-0077
- DIALOGUE-AUTO-0026

## Referências

- Documento interno: `EDITORIAL_RULES.md`

## Tags

- case
- auto
- exclusao
- cobertura

## Veja também

- `14_FAQ/AUTO/FAQ-AUTO-0075.md`

## Histórico de revisão

| Data | Versão | Autor | Alteração |
| --- | --- | --- | --- |
| 2026-06-25 | 0.1 | JP-BR Insurance Knowledge Base | Criação inicial. |
