# AUTO_CASES_0037_0044 - Estudos de caso AUTO

## Controle

- Versão: 0.2
- Data: 2026-06-26
- Autor: JP-BR Insurance Knowledge Base
- Histórico: Consolidação atualizada com limite comercial/compliance.

## Conteúdo consolidado

# CASE-AUTO-0037 - Cliente quer reduzir preço sem perder proteção essencial

## Controle

- Versão: 0.2
- Data: 2026-06-25
- Autor: JP-BR Insurance Knowledge Base
- Histórico: Criação inicial; inclusão de limite comercial/compliance.

## Código

CASE-AUTO-0037

## Produto ou categoria

AUTO

## Situação

Cliente quer reduzir preço sem perder proteção essencial.

## Problema

Cliente pode manter contrato desatualizado, interpretar resumo como contrato completo ou perder prazo importante por falta de acompanhamento.

## Análise

Atendimento recorrente, conferência de dados, revisão de cobertura e controle de vencimento reduzem riscos comerciais e operacionais.

## Orientação ao corretor

Revisar dados e coberturas com método, registrar mudanças e orientar o cliente antes de vencimento, cancelamento ou sinistro.

## Como explicar ao cliente brasileiro

Vamos manter seu seguro atualizado para evitar surpresa quando você precisar usar ou renovar.

## Resultado esperado

Cliente mantém contrato correto, entende o processo e recebe acompanhamento profissional depois da venda.

## Limite comercial/compliance

O corretor deve usar este caso como orientação de atendimento, sem prometer cobertura, valor, prazo de pagamento, aceitação de responsabilidade ou resultado de análise. A decisão final depende da apólice, documentos, circunstâncias do evento e avaliação da seguradora.

## Termos relacionados

- AUTO-0081 - 補償の見直し - Revisão das coberturas
- AUTO-0082 - 保険料の見直し - Revisão do prêmio do seguro
- AUTO-0083 - 契約条件 - Condições do contrato

## FAQs relacionadas

- FAQ-AUTO-0078
- FAQ-AUTO-0079
- FAQ-AUTO-0080

## Diálogos relacionados

- DIALOGUE-AUTO-0027

## Referências cruzadas

- AUTO-0081
- AUTO-0082
- AUTO-0083
- FAQ-AUTO-0078
- FAQ-AUTO-0079
- FAQ-AUTO-0080
- DIALOGUE-AUTO-0027

## Referências

- Documento interno: `EDITORIAL_RULES.md`

## Tags

- case
- auto
- atendimento
- revisao

## Veja também

- `14_FAQ/AUTO/FAQ-AUTO-0078.md`

## Histórico de revisão

| Data | Versão | Autor | Alteração |
| --- | --- | --- | --- |
| 2026-06-25 | 0.1 | JP-BR Insurance Knowledge Base | Criação inicial. |
| 2026-06-26 | 0.2 | JP-BR Insurance Knowledge Base | Inclusão de limite comercial/compliance para uso seguro em atendimento e NotebookLM. |

---

# CASE-AUTO-0038 - Cliente não leu informações importantes antes de contratar

## Controle

- Versão: 0.1
- Data: 2026-06-25
- Autor: JP-BR Insurance Knowledge Base
- Histórico: Criação inicial.

## Código

CASE-AUTO-0038

## Produto ou categoria

AUTO

## Situação

Cliente não leu informações importantes antes de contratar.

## Problema

Cliente pode manter contrato desatualizado, interpretar resumo como contrato completo ou perder prazo importante por falta de acompanhamento.

## Análise

Atendimento recorrente, conferência de dados, revisão de cobertura e controle de vencimento reduzem riscos comerciais e operacionais.

## Orientação ao corretor

Revisar dados e coberturas com método, registrar mudanças e orientar o cliente antes de vencimento, cancelamento ou sinistro.

## Como explicar ao cliente brasileiro

Antes de contratar, vamos conferir juntos as informações importantes, o resumo do contrato e os alertas. Assim você entende o que está coberto, o que pode ficar fora e o que precisa atualizar depois.

## Próximo passo recomendado

Confirmar com o cliente: condutores reais, uso do veículo, dados do carro, coberturas escolhidas, franquia, exclusões principais e prazo de renovação.

## Limite comercial/compliance

O corretor deve explicar os pontos importantes e registrar a orientação, mas a cobertura efetiva depende das condições do contrato e da análise da seguradora em caso de sinistro.

## Resultado esperado

Cliente mantém contrato correto, entende o processo e recebe acompanhamento profissional depois da venda.

## Termos relacionados

- AUTO-0084 - 重要事項説明 - Explicação de informações importantes
- AUTO-0085 - 契約概要 - Resumo do contrato
- AUTO-0086 - 注意喚起情報 - Informações de alerta
- AUTO-0087 - 契約確認 - Confirmação do contrato

## FAQs relacionadas

- FAQ-AUTO-0081
- FAQ-AUTO-0082
- FAQ-AUTO-0083
- FAQ-AUTO-0084

## Diálogos relacionados

- DIALOGUE-AUTO-0028

## Referências cruzadas

- AUTO-0084
- AUTO-0085
- AUTO-0086
- AUTO-0087
- FAQ-AUTO-0081
- FAQ-AUTO-0082
- FAQ-AUTO-0083
- FAQ-AUTO-0084
- DIALOGUE-AUTO-0028

## Referências

- Documento interno: `EDITORIAL_RULES.md`

## Tags

- case
- auto
- atendimento
- revisao

## Veja também

- `14_FAQ/AUTO/FAQ-AUTO-0081.md`

## Histórico de revisão

| Data | Versão | Autor | Alteração |
| --- | --- | --- | --- |
| 2026-06-25 | 0.1 | JP-BR Insurance Knowledge Base | Criação inicial. |
| 2026-06-26 | 0.2 | JP-BR Insurance Knowledge Base | Reforço de explicação prática, próximo passo e limite comercial. |

---

# CASE-AUTO-0039 - Dados do cliente, veículo ou condutor estão incorretos

## Controle

- Versão: 0.2
- Data: 2026-06-25
- Autor: JP-BR Insurance Knowledge Base
- Histórico: Criação inicial; inclusão de limite comercial/compliance.

## Código

CASE-AUTO-0039

## Produto ou categoria

AUTO

## Situação

Dados do cliente, veículo ou condutor estão incorretos.

## Problema

Cliente pode manter contrato desatualizado, interpretar resumo como contrato completo ou perder prazo importante por falta de acompanhamento.

## Análise

Atendimento recorrente, conferência de dados, revisão de cobertura e controle de vencimento reduzem riscos comerciais e operacionais.

## Orientação ao corretor

Revisar dados e coberturas com método, registrar mudanças e orientar o cliente antes de vencimento, cancelamento ou sinistro.

## Como explicar ao cliente brasileiro

Vamos manter seu seguro atualizado para evitar surpresa quando você precisar usar ou renovar.

## Resultado esperado

Cliente mantém contrato correto, entende o processo e recebe acompanhamento profissional depois da venda.

## Limite comercial/compliance

O corretor deve usar este caso como orientação de atendimento, sem prometer cobertura, valor, prazo de pagamento, aceitação de responsabilidade ou resultado de análise. A decisão final depende da apólice, documentos, circunstâncias do evento e avaliação da seguradora.

## Termos relacionados

- AUTO-0088 - 顧客情報 - Informações do cliente
- AUTO-0089 - 車両情報 - Informações do veículo
- AUTO-0090 - 運転者情報 - Informações do condutor

## FAQs relacionadas

- FAQ-AUTO-0085
- FAQ-AUTO-0086
- FAQ-AUTO-0087

## Diálogos relacionados

- DIALOGUE-AUTO-0029

## Referências cruzadas

- AUTO-0088
- AUTO-0089
- AUTO-0090
- FAQ-AUTO-0085
- FAQ-AUTO-0086
- FAQ-AUTO-0087
- DIALOGUE-AUTO-0029

## Referências

- Documento interno: `EDITORIAL_RULES.md`

## Tags

- case
- auto
- atendimento
- revisao

## Veja também

- `14_FAQ/AUTO/FAQ-AUTO-0085.md`

## Histórico de revisão

| Data | Versão | Autor | Alteração |
| --- | --- | --- | --- |
| 2026-06-25 | 0.1 | JP-BR Insurance Knowledge Base | Criação inicial. |
| 2026-06-26 | 0.2 | JP-BR Insurance Knowledge Base | Inclusão de limite comercial/compliance para uso seguro em atendimento e NotebookLM. |

---

# CASE-AUTO-0040 - Cliente não sabe o fluxo correto após acidente

## Controle

- Versão: 0.2
- Data: 2026-06-25
- Autor: JP-BR Insurance Knowledge Base
- Histórico: Criação inicial; inclusão de limite comercial/compliance.

## Código

CASE-AUTO-0040

## Produto ou categoria

AUTO

## Situação

Cliente não sabe o fluxo correto após acidente.

## Problema

Cliente pode manter contrato desatualizado, interpretar resumo como contrato completo ou perder prazo importante por falta de acompanhamento.

## Análise

Atendimento recorrente, conferência de dados, revisão de cobertura e controle de vencimento reduzem riscos comerciais e operacionais.

## Orientação ao corretor

Revisar dados e coberturas com método, registrar mudanças e orientar o cliente antes de vencimento, cancelamento ou sinistro.

## Como explicar ao cliente brasileiro

Vamos manter seu seguro atualizado para evitar surpresa quando você precisar usar ou renovar.

## Resultado esperado

Cliente mantém contrato correto, entende o processo e recebe acompanhamento profissional depois da venda.

## Limite comercial/compliance

O corretor deve usar este caso como orientação de atendimento, sem prometer cobertura, valor, prazo de pagamento, aceitação de responsabilidade ou resultado de análise. A decisão final depende da apólice, documentos, circunstâncias do evento e avaliação da seguradora.

## Termos relacionados

- AUTO-0091 - 事故対応フロー - Fluxo de atendimento em acidente
- AUTO-0092 - 必要書類 - Documentos necessários

## FAQs relacionadas

- FAQ-AUTO-0088
- FAQ-AUTO-0089

## Diálogos relacionados

- DIALOGUE-AUTO-0030

## Referências cruzadas

- AUTO-0091
- AUTO-0092
- FAQ-AUTO-0088
- FAQ-AUTO-0089
- DIALOGUE-AUTO-0030

## Referências

- Documento interno: `EDITORIAL_RULES.md`

## Tags

- case
- auto
- atendimento
- revisao

## Veja também

- `14_FAQ/AUTO/FAQ-AUTO-0088.md`

## Histórico de revisão

| Data | Versão | Autor | Alteração |
| --- | --- | --- | --- |
| 2026-06-25 | 0.1 | JP-BR Insurance Knowledge Base | Criação inicial. |
| 2026-06-26 | 0.2 | JP-BR Insurance Knowledge Base | Inclusão de limite comercial/compliance para uso seguro em atendimento e NotebookLM. |

---

# CASE-AUTO-0041 - Seguradora pede confirmação de identidade e documentos

## Controle

- Versão: 0.2
- Data: 2026-06-25
- Autor: JP-BR Insurance Knowledge Base
- Histórico: Criação inicial; inclusão de limite comercial/compliance.

## Código

CASE-AUTO-0041

## Produto ou categoria

AUTO

## Situação

Seguradora pede confirmação de identidade e documentos.

## Problema

Cliente pode manter contrato desatualizado, interpretar resumo como contrato completo ou perder prazo importante por falta de acompanhamento.

## Análise

Atendimento recorrente, conferência de dados, revisão de cobertura e controle de vencimento reduzem riscos comerciais e operacionais.

## Orientação ao corretor

Revisar dados e coberturas com método, registrar mudanças e orientar o cliente antes de vencimento, cancelamento ou sinistro.

## Como explicar ao cliente brasileiro

Vamos manter seu seguro atualizado para evitar surpresa quando você precisar usar ou renovar.

## Resultado esperado

Cliente mantém contrato correto, entende o processo e recebe acompanhamento profissional depois da venda.

## Limite comercial/compliance

O corretor deve usar este caso como orientação de atendimento, sem prometer cobertura, valor, prazo de pagamento, aceitação de responsabilidade ou resultado de análise. A decisão final depende da apólice, documentos, circunstâncias do evento e avaliação da seguradora.

## Termos relacionados

- AUTO-0092 - 必要書類 - Documentos necessários
- AUTO-0093 - 本人確認 - Confirmação de identidade

## FAQs relacionadas

- FAQ-AUTO-0089
- FAQ-AUTO-0090

## Diálogos relacionados

- DIALOGUE-AUTO-0030

## Referências cruzadas

- AUTO-0092
- AUTO-0093
- FAQ-AUTO-0089
- FAQ-AUTO-0090
- DIALOGUE-AUTO-0030

## Referências

- Documento interno: `EDITORIAL_RULES.md`

## Tags

- case
- auto
- atendimento
- revisao

## Veja também

- `14_FAQ/AUTO/FAQ-AUTO-0089.md`

## Histórico de revisão

| Data | Versão | Autor | Alteração |
| --- | --- | --- | --- |
| 2026-06-25 | 0.1 | JP-BR Insurance Knowledge Base | Criação inicial. |
| 2026-06-26 | 0.2 | JP-BR Insurance Knowledge Base | Inclusão de limite comercial/compliance para uso seguro em atendimento e NotebookLM. |

---

# CASE-AUTO-0042 - Cliente mudou endereço, contato ou titularidade

## Controle

- Versão: 0.2
- Data: 2026-06-25
- Autor: JP-BR Insurance Knowledge Base
- Histórico: Criação inicial; inclusão de limite comercial/compliance.

## Código

CASE-AUTO-0042

## Produto ou categoria

AUTO

## Situação

Cliente mudou endereço, contato ou titularidade.

## Problema

Cliente pode manter contrato desatualizado, interpretar resumo como contrato completo ou perder prazo importante por falta de acompanhamento.

## Análise

Atendimento recorrente, conferência de dados, revisão de cobertura e controle de vencimento reduzem riscos comerciais e operacionais.

## Orientação ao corretor

Revisar dados e coberturas com método, registrar mudanças e orientar o cliente antes de vencimento, cancelamento ou sinistro.

## Como explicar ao cliente brasileiro

Vamos manter seu seguro atualizado para evitar surpresa quando você precisar usar ou renovar.

## Resultado esperado

Cliente mantém contrato correto, entende o processo e recebe acompanhamento profissional depois da venda.

## Limite comercial/compliance

O corretor deve usar este caso como orientação de atendimento, sem prometer cobertura, valor, prazo de pagamento, aceitação de responsabilidade ou resultado de análise. A decisão final depende da apólice, documentos, circunstâncias do evento e avaliação da seguradora.

## Termos relacionados

- AUTO-0094 - 住所変更 - Alteração de endereço
- AUTO-0095 - 連絡先変更 - Alteração de contato
- AUTO-0096 - 名義変更 - Alteração de titularidade

## FAQs relacionadas

- FAQ-AUTO-0091
- FAQ-AUTO-0092
- FAQ-AUTO-0093

## Diálogos relacionados

- DIALOGUE-AUTO-0031

## Referências cruzadas

- AUTO-0094
- AUTO-0095
- AUTO-0096
- FAQ-AUTO-0091
- FAQ-AUTO-0092
- FAQ-AUTO-0093
- DIALOGUE-AUTO-0031

## Referências

- Documento interno: `EDITORIAL_RULES.md`

## Tags

- case
- auto
- atendimento
- revisao

## Veja também

- `14_FAQ/AUTO/FAQ-AUTO-0091.md`

## Histórico de revisão

| Data | Versão | Autor | Alteração |
| --- | --- | --- | --- |
| 2026-06-25 | 0.1 | JP-BR Insurance Knowledge Base | Criação inicial. |
| 2026-06-26 | 0.2 | JP-BR Insurance Knowledge Base | Inclusão de limite comercial/compliance para uso seguro em atendimento e NotebookLM. |

---

# CASE-AUTO-0043 - Cliente quer cancelar perto da renovação

## Controle

- Versão: 0.2
- Data: 2026-06-25
- Autor: JP-BR Insurance Knowledge Base
- Histórico: Criação inicial; inclusão de limite comercial/compliance.

## Código

CASE-AUTO-0043

## Produto ou categoria

AUTO

## Situação

Cliente quer cancelar perto da renovação.

## Problema

Cliente pode manter contrato desatualizado, interpretar resumo como contrato completo ou perder prazo importante por falta de acompanhamento.

## Análise

Atendimento recorrente, conferência de dados, revisão de cobertura e controle de vencimento reduzem riscos comerciais e operacionais.

## Orientação ao corretor

Revisar dados e coberturas com método, registrar mudanças e orientar o cliente antes de vencimento, cancelamento ou sinistro.

## Como explicar ao cliente brasileiro

Vamos manter seu seguro atualizado para evitar surpresa quando você precisar usar ou renovar.

## Resultado esperado

Cliente mantém contrato correto, entende o processo e recebe acompanhamento profissional depois da venda.

## Limite comercial/compliance

O corretor deve usar este caso como orientação de atendimento, sem prometer cobertura, valor, prazo de pagamento, aceitação de responsabilidade ou resultado de análise. A decisão final depende da apólice, documentos, circunstâncias do evento e avaliação da seguradora.

## Termos relacionados

- AUTO-0097 - 解約手続き - Procedimento de cancelamento
- AUTO-0098 - 更新案内 - Aviso de renovação
- AUTO-0099 - 満期管理 - Controle de vencimento

## FAQs relacionadas

- FAQ-AUTO-0094
- FAQ-AUTO-0095
- FAQ-AUTO-0096

## Diálogos relacionados

- DIALOGUE-AUTO-0032

## Referências cruzadas

- AUTO-0097
- AUTO-0098
- AUTO-0099
- FAQ-AUTO-0094
- FAQ-AUTO-0095
- FAQ-AUTO-0096
- DIALOGUE-AUTO-0032

## Referências

- Documento interno: `EDITORIAL_RULES.md`

## Tags

- case
- auto
- atendimento
- revisao

## Veja também

- `14_FAQ/AUTO/FAQ-AUTO-0094.md`

## Histórico de revisão

| Data | Versão | Autor | Alteração |
| --- | --- | --- | --- |
| 2026-06-25 | 0.1 | JP-BR Insurance Knowledge Base | Criação inicial. |
| 2026-06-26 | 0.2 | JP-BR Insurance Knowledge Base | Inclusão de limite comercial/compliance para uso seguro em atendimento e NotebookLM. |

---

# CASE-AUTO-0044 - Pós-venda evita vencimento e problemas futuros

## Controle

- Versão: 0.2
- Data: 2026-06-25
- Autor: JP-BR Insurance Knowledge Base
- Histórico: Criação inicial; inclusão de limite comercial/compliance.

## Código

CASE-AUTO-0044

## Produto ou categoria

AUTO

## Situação

Pós-venda evita vencimento e problemas futuros.

## Problema

Cliente pode manter contrato desatualizado, interpretar resumo como contrato completo ou perder prazo importante por falta de acompanhamento.

## Análise

Atendimento recorrente, conferência de dados, revisão de cobertura e controle de vencimento reduzem riscos comerciais e operacionais.

## Orientação ao corretor

Revisar dados e coberturas com método, registrar mudanças e orientar o cliente antes de vencimento, cancelamento ou sinistro.

## Como explicar ao cliente brasileiro

Vamos manter seu seguro atualizado para evitar surpresa quando você precisar usar ou renovar.

## Resultado esperado

Cliente mantém contrato correto, entende o processo e recebe acompanhamento profissional depois da venda.

## Limite comercial/compliance

O corretor deve usar este caso como orientação de atendimento, sem prometer cobertura, valor, prazo de pagamento, aceitação de responsabilidade ou resultado de análise. A decisão final depende da apólice, documentos, circunstâncias do evento e avaliação da seguradora.

## Termos relacionados

- AUTO-0098 - 更新案内 - Aviso de renovação
- AUTO-0099 - 満期管理 - Controle de vencimento
- AUTO-0100 - アフターサービス - Atendimento pós-venda

## FAQs relacionadas

- FAQ-AUTO-0095
- FAQ-AUTO-0096
- FAQ-AUTO-0097

## Diálogos relacionados

- DIALOGUE-AUTO-0032

## Referências cruzadas

- AUTO-0098
- AUTO-0099
- AUTO-0100
- FAQ-AUTO-0095
- FAQ-AUTO-0096
- FAQ-AUTO-0097
- DIALOGUE-AUTO-0032

## Referências

- Documento interno: `EDITORIAL_RULES.md`

## Tags

- case
- auto
- atendimento
- revisao

## Veja também

- `14_FAQ/AUTO/FAQ-AUTO-0095.md`

## Histórico de revisão

| Data | Versão | Autor | Alteração |
| --- | --- | --- | --- |
| 2026-06-25 | 0.1 | JP-BR Insurance Knowledge Base | Criação inicial. |
| 2026-06-26 | 0.2 | JP-BR Insurance Knowledge Base | Inclusão de limite comercial/compliance para uso seguro em atendimento e NotebookLM. |

## Histórico de revisão

| Data | Versão | Autor | Alteração |
| --- | --- | --- | --- |
| 2026-06-26 | 0.2 | JP-BR Insurance Knowledge Base | Regeneração do pacote com normalização de compliance dos estudos de caso AUTO. |
